...
# Form visibility state (essential for the MoMo forms)
if 'show_deposit_form' not in st.session_state:
    st.session_state.show_deposit_form = False
if 'show_withdrawal_form' not in st.session_state:
...
def set_view(view_name):
    ...
    # Reset form visibility when changing view
    st.session_state.show_deposit_form = False
    st.session_state.show_withdrawal_form = False
...
def home_view():
    ...
    with col1:
        # **CORRECTED:** Added unique key
        st.button("🙋 User for Betting", on_click=set_view, args=('user_panel',), use_container_width=True, type="primary", key='home_user_btn')
    
    with col2:
        # **CORRECTED:** Added unique key
        st.button("⚙️ Admin Panel", on_click=set_view, args=('admin_panel',), use_container_width=True, key='home_admin_btn')
    st.markdown("---")
...
def admin_panel_view():
    ...
    # **CORRECTED:** Added unique key
    st.button("🏠 Back to Home", on_click=set_view, args=('home',), key='admin_back_btn')
    st.markdown("---")
...
def user_panel_view():
    ...
    # **CORRECTED:** Added unique key
    st.button("🏠 Back to Home", on_click=set_view, args=('home',), key='user_back_btn')
    st.markdown("---")
    ...
    with st.sidebar:
        ...
        # Deposit Form Button
        # **CORRECTED:** Added unique key
        if st.button("💰 Deposit via Mobile Money", use_container_width=True, key='deposit_btn_sidebar'):
            st.session_state.show_deposit_form = True
        
        # Withdrawal Form Button
        # **CORRECTED:** Added unique key
        if st.button("🏦 Withdraw Funds", use_container_width=True, key='withdraw_btn_sidebar'):
    ...
    if st.session_state.get('show_deposit_form', False):
        with st.form("deposit_form"):
            ...
            # **CORRECTED:** Added unique keys to form inputs
            dep_phone = st.text_input("Mobile Phone Number (e.g., 078xxxxxxx)", value="078xxxxxxx", key='dep_phone')
            dep_pin = st.text_input("Mobile Money PIN (Simulated)", type="password", key='dep_pin') 
            dep_amount = st.number_input("Deposit Amount ($)", min_value=1.00, value=50.00, step=1.00, key='dep_amount')
            
            submitted = st.form_submit_button("Confirm Deposit", type="primary") # Form buttons are automatically keyed
            if submitted:
    ...
    if st.session_state.get('show_withdrawal_form', False):
        with st.form("withdrawal_form"):
            ...
            # **CORRECTED:** Added unique keys to form inputs
            wdraw_phone = st.text_input("Mobile Phone Number to Receive Funds", value="078xxxxxxx", key='wdraw_phone')
            wdraw_amount = st.number_input("Withdrawal Amount ($)", min_value=1.00, max_value=st.session_state.balance, value=10.00, step=1.00, key='wdraw_amount')
            
            submitted = st.form_submit_button("Confirm Withdrawal", type="primary") # Form buttons are automatically keyed
            if submitted:
    ...
    with col_bet:
        ...
        # **CORRECTED:** Added unique key
        bet_input = st.number_input("Enter Bet Amount ($)", min_value=1.00, max_value=st.session_state.balance, step=1.00, value=5.00, key='bet_input_main', disabled=st.session_state.game_running)
        
        # **CORRECTED:** Added unique key
        st.button("🚀 Place Bet", on_click=start_game, args=(bet_input,), disabled=st.session_state.game_running, type="secondary", key='place_bet_btn')
        
        if st.session_state.game_running:
            # **CORRECTED:** Added unique key
            st.button(f"💰 Cash Out at {st.session_state.current_multiplier:.2f}x", on_click=cashout, disabled=not st.session_state.can_cashout, type="primary", use_container_width=True, key='cashout_btn')
...
st.set_page_config(page_title="Aviator Split Panel Sim", layout="wide")

if st.session_state.current_view == 'home':
    home_view()
elif st.session_state.current_view == 'admin_panel':
    admin_panel_view()
elif st.session_state.current_view == 'user_panel':
    user_panel_view()